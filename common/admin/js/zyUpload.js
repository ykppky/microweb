		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<div class="filePicker">上传封面图片</div>';
					...
					// 创建不带有拖动的html
					html += '<form id="uploadForm" action="'+para.url+'" method="post" enctype="multipart/form-data">';
					//html += '<input type="hidden" name="folder" value="<?php echo $folder?>">' 
					html += '	<div class="upload_box">';
		            ...
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		           // html += '					<div class="webuploader_pick">选择封面图片</div>';
		            html += '					<div class="upload_btn">开始上传</div>';
		            html += '				</div>';
		            html += '			</div>';
				    ...
				    html += '							<img class="upload_image" src="images/zyUpload/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
					...
					html += '</form>';
					html += '		<div id="uploadInf" class="upload_inf"></div>';
				//	html += '		<input type="hidden" id="uploadNum" class="upload_Num" name="uploadNum"/>';
				}
			...
			this.createCorePlug = function(){
				...
				//		$("#uploadInf").append("<input type='hidden' name='picname"+file.index+"' value='"+response.trim()+"' />");
				/*		var numInf = $("#uploadNum").val();
						if( numInf == '')
						{
							numInf = "picname"+file.index;
						}
						else
						{
							numInf += ";picname"+file.index;
						}
						$("#uploadNum").val(numInf);*/
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
