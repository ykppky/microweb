...
<?php
	...
	class ValidationCode {
		...
		function __construct($width=150, $height=50, $codeNum=4) {
			...
			$this->font = dirname(dirname(dirname( __FILE__ )))."/common/font/Elephant.ttf";
			$this->fontSize = 20;
		}
		...
		private function outputText() {       //随机颜色、随机摆放、随机字符串向图像中输出
			for ($i=0;$i<=$this->codeNum;$i++) {
   				 ...
   				 $y = rand( $this->fontSize+5,$this->height-$this->fontSize);
				 //imagechar($this->image, 55, $x, $y, $this->checkCode[$i], $bg_color);
				 imagettftext( $this->image, $this->fontSize, mt_rand(-30,30), $x, $y, $bg_color, $this->font, $this->checkCode[$i]);
 			  }
		}
